...
export function NumericKeypadInput({
    ...
    // displayValue mirrors the internal state while open, but we sync with props
    ...
    const handleInput = (input: string): void => {
        ...
        switch (input) {
            ...
            case 'DEL':
                ...
                break;
            case '000':
                if (newValue.length + 3 <= maxLength) {
                    newValue += '000';
                }
                break;
            default:
                                ...
                                <div className="bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-950 rounded-t-[2rem] shadow-[0_-10px_60px_rgba(0,0,0,0.15)] overflow-hidden">
                                    ...
                                    <div className="px-6 py-5">
                                        <div className="text-center">
                                            <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mb-3 tracking-wide">{label || 'Nomor'}</p>
                                            <div className="flex items-baseline justify-center gap-1.5 min-h-[3.5rem]">
                                                <span className={`text-[2.5rem] leading-none font-bold tracking-widest break-all transition-colors ${!displayValue
