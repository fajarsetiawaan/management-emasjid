...
export default function FundCards({ funds }: FundCardsProps) {
    return (
        <section className="mb-8">
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 -mx-4 px-4 snap-x hide-scrollbar">
                ...
                {funds.map((fund) => (
                    <div key={fund.id} className="snap-center shrink-0 w-[160px] group relative">
                        ...
                        <div className="relative h-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[1.8rem] p-5 shadow-lg shadow-indigo-500/5 flex flex-col justify-between hover:border-indigo-500/50 dark:hover:border-indigo-500/50 transition-colors min-h-[180px]">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider ${fund.type === 'OPERASIONAL' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                                    {fund.type}
                                </span>
                            </div>
                            <div>
                                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1 line-clamp-2 leading-tight">{fund.name}</p>
