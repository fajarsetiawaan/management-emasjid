...
export default function SettingsPage() {
    ...
    const SettingsGroup = ({ title, children }: { title?: string, children: React.ReactNode }) => (
        <div className="mb-6">
            {title && <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-4">{title}</h3>}
            <div className="bg-white border-y border-slate-100 sm:border sm:rounded-2xl overflow-hidden divide-y divide-slate-50">
                {children}
            </div>
        </div>
    );
    ...
    const SettingsItem = ({
        ...
        <button
            onClick={onClick}
            className={`w-full flex items-center justify-between p-4 transition-colors hover:bg-slate-50 active:bg-slate-100
                ${isDestructive ? 'hover:bg-rose-50' : ''}
            `}
        >
            <div className="flex items-center gap-3.5">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center 
                    ${isDestructive ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-600'}
                `}>
                    <Icon size={18} />
                </div>
                <span className={`text-sm font-medium ${isDestructive ? 'text-rose-600' : 'text-slate-700'}`}>
                    {label}
                </span>
            </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-24">
            ...
            {/* A. Profile Header */}
            <header className="bg-white px-6 pt-8 pb-8 flex items-center justify-between border-b border-slate-100">
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <div className="w-16 h-16 rounded-full bg-slate-200 border-2 border-white shadow-md overflow-hidden flex items-center justify-center text-slate-400">
                            <User size={32} />
                        </div>
                        <div className="absolute -bottom-1 -right-1 bg-amber-400 text-white text-[10px] font-bold px-2 py-0.5 rounded-full border border-white flex items-center gap-0.5 shadow-sm">
                            <Crown size={10} fill="currentColor" />
                            {MOCK_USER.role === 'OWNER' ? 'Ketua DKM' : 'Pengurus'}
                        </div>
                    </div>
                    <div>
                        <h1 className="text-lg font-bold text-slate-800 leading-tight">{MOCK_USER.name}</h1>
                        <p className="text-sm text-slate-400">{MOCK_USER.email}</p>
                    </div>
                </div>
                <button
                    onClick={() => handleNavigate('Edit Profil')}
                    className="w-9 h-9 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors"
                >
                    <Edit2 size={16} />
                </button>
            </header>
            ...
            <div className="px-4 -mt-4 relative z-10 space-y-6">
                ...
                {/* B. Subscription Card */}
                <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-lg shadow-emerald-500/20 flex flex-col gap-4 relative overflow-hidden">
                    {/* Decorative Circles */}
                ...
                <SettingsGroup title="Preferensi Aplikasi">
                    ...
                    <SettingsItem
                        ...
                        value={theme === 'auto' ? 'Otomatis' : theme === 'dark' ? 'Gelap' : 'Terang'}
                    />
                </SettingsGroup>
                    ...
                    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-slate-800">Pilih Tampilan</h3>
                                <button onClick={() => setShowThemeModal(false)} className="p-2 bg-slate-100 rounded-full text-slate-500 hover:text-slate-800">
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <div className="space-y-3">
                                {[
                                    ...
                                    { id: 'auto', label: 'Otomatis (System)', icon: '⚙️' },
                                ].map((item) => (
                                    <button
                                        key={item.id}
                ...
                {showLogoutModal && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
                        ...
                        <motion.div
                            ...
                            className="bg-white w-full max-w-sm rounded-2xl p-6 relative z-10 text-center shadow-xl"
                        >
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className="flex-1 py-3 rounded-xl bg-rose-600 text-white font-bold hover:bg-rose-700 shadow-lg shadow-rose-200 transition-colors"
                                >
                                    Ya, Keluar
                                </button>
                            </div>
